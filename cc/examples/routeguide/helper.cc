...
#include <string>
#include <vector>
#include "proto/examples/routeguide/routeguide.grpc.pb.h"
...
std::string GetDbFileContent(int argc, char** argv) {
  ...
  // Assume arg 1 and 2 are the hostname and port.
  if (argc > 3) {
    std::string argv_3 = argv[3];
    size_t start_position = argv_3.find(arg_str);
    if (start_position != std::string::npos) {
      start_position += arg_str.size();
      if (argv_3[start_position] == ' ' ||
          argv_3[start_position] == '=') {
        db_path = argv_3.substr(start_position + 1);
      }
    }
  } else {
    db_path = "proto/examples/routeguide/routeguide_features.json";
  }
